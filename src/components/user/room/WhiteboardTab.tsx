...
const WhiteboardTab: React.FC<ExcalidrawPageProps> = ({ roomId, canDraw }) => {
  ...
  const sendUpdate = useCallback((elements: any[], appState: any, files: any) => {
    if (!canDraw || !isTabVisible) return;
    ...
    console.log('Sending whiteboard update with', elements.length, 'elements');
    roomSocketService.updateWhiteboard(updateData);
        ...
        setTimeout(() => {
          try {
            ...
            excalidrawAPI.updateScene({
              elements: data.elements,
              appState: data.appState || {},
              captureUpdate: false // Don't capture remote updates in history
            });
          } catch (error) {
            console.warn('Failed to update scene:', error);
          }
  ...
  // Handle Excalidraw changes with proper drawing state detection
  const handleChange = useCallback((newElements: any[], newAppState: any, newFiles: any) => {
    // Skip if we're receiving an update from another user
    if (isReceivingUpdate.current) {
      return;
    }
    ...
    // Check if user is actively drawing by looking for incomplete elements
    const hasIncompleteElements = newElements.some(element => {
      // Elements being drawn have specific properties that indicate they're incomplete
      return element.isBeingDrawn || 
             (element.type === 'draw' && element.points && element.points.length === 1) ||
             (element.type === 'line' && element.points && element.points.length < 2) ||
             (element.type === 'arrow' && element.points && element.points.length < 2);
    });

    const isCurrentlyDrawing = newAppState.activeTool?.type && 
                               newAppState.activeTool?.type !== 'selection' &&
                               newAppState.isLoading === false &&
                               newAppState.draggingElement !== null;

    // Detect drawing state changes
    if (isCurrentlyDrawing || hasIncompleteElements) {
      if (!isDrawingRef.current) {
        console.log('Started drawing - sending immediate update');
        isDrawingRef.current = true;
        sendUpdate(newElements, newAppState, newFiles);
      } else {
        // While drawing, send updates every 150ms to show progress
        if (now - lastUpdateTimeRef.current > 150) {
          console.log('Drawing in progress - sending update');
          sendUpdate(newElements, newAppState, newFiles);
        }
      }
      ...
      drawingTimeoutRef.current = setTimeout(() => {
        if (isDrawingRef.current) {
          console.log('Drawing finished - sending final update');
          isDrawingRef.current = false;
          sendUpdate(currentElementsRef.current, currentAppStateRef.current, currentFilesRef.current);
        }
      }, 200);
    } else {
      // Not drawing - send update if drawing just finished or if enough time passed
      if (isDrawingRef.current || now - lastUpdateTimeRef.current > 500) {
        console.log('Sending update - drawing finished or timeout reached');
        isDrawingRef.current = false;
        sendUpdate(newElements, newAppState, newFiles);
      }
    }
  }, [canDraw, isTabVisible, sendUpdate]);
  ...
  const handleClearCanvas = useCallback(() => {
    ...
    if (window.confirm('Are you sure you want to clear the whiteboard? This action cannot be undone.')) {
      ...
      sendUpdate(emptyElements, currentAppStateRef.current, currentFilesRef.current);
    }
  }, [canDraw, excalidrawAPI, sendUpdate]);
